...
<body>
	...
	<div id="artDetail" class="container ">
        ...
        <%
        	Plate list = new Plate();
        	list = plateDao.selectPlate(request.getParameter("plateid").toString());
        	session.setAttribute("changePic_Id", list.getPlate_id());
        %>
	<form class="form" action = "UserloginServlet?method=7&plateid=<%=list.getPlate_id() %>" method = "post" >
        <table style="width:1200px;margin:auto;">
        	...
        	<tr>
        		<td><input type="text" name="plate_id" value="<%=list.getPlate_id() %>"></td>
        		<td><input type="text" name="plate_name" value="<%=list.getPlate_name()%>"></td>
        		<td><input type="text" name="motto_one" value="<%=list.getMotto_one() %>"></td>
        		<td><input type="text" name="motto_two" value="<%=list.getMotto_two() %>"></td>
        		<td><input type="text" name="motto_three" value="<%=list.getMotto_three() %>"></td>
        		<td><input type="text" name="plate_cont" value="<%=list.getPlate_cont() %>"></td>
        		<td><button type="submit">保存</button></td>
        	</tr>
        </table>
    </form>
    <form action = "UserloginServlet?method=8&plateid=<%=list.getPlate_id() %>" method = "post"  enctype="multipart/form-data"
    		style="width:400px;margin:auto;">
    	<tr>
        	<td>更换头像——请选择要上传的文件</td>
    	</tr>
    	<tr>
    	<input type="file" class="label label-primary" name="upfile" size="50">
		
		<div style="width:300px; margin:auto;">
			<button type="submit"  class="btn btn-primary" style="width:300px;margin-top: 10px;">确认选择</button>
		</div>
    	</tr>
    </form>
    </div>
