...
<body >
        ...
        <div id="artDetail" class="container ">
            <div class="card">
                ...
                User user = userinfolist.get(0);
                 %>
                <!-- 上传图片 -->
                		<form action="dome.jsp?userid=<%=userId %>" method="post">
                		<div class="headimagediv"  >
                		<div class="change">
                			<img  class="headimage" src="<%=user.getUserpicture_url()%>">
                			<div class="rsp"></div>
                			<div class="changeimgtext">更换头像</div>
                		</div>
                		</div>
				        ...
				        <span class="user-info" style="color:#000000"><%=user.getUser_name() %></span>
				        <!-- <span class="warning-info" style="color:#9e9e9e">*请填写仅包含英文字母的用户名</span> -->
				      </div>
				      
				      <div class="row1">
				      <span class="label">个性签名</span>
				       <span class="user-info" style="color:#000000;"><%=user.getSlogan() %></span><!-- &nbsp;&nbsp;<a href="#0" onclick="edit(this);" >更改</a>  -->
				      </div>
				      
				      <%-- <div class="row1">
				      <span class="label">更改密码</span>
				      <a href="dome.jsp?userid=<%=userId %>&change=2" style="margin-left: 80px;">点击更改密码</a>
				      </div> --%>
				     
				      <div class="row1">
				        <span class="label">院系年级</span>
				        <span class="user-info" style="color:#000000"><%=user.getUser_faculty() %></span>
				      </div> 
				     
				      <div class="row1">
				      <span class="label">电子邮箱</span>
				      <span class="user-info" style="color:#000000"><%=user.getUser_email() %></span>
				      </div>
				      <div class="row1"><span class="label">性别&nbsp;&nbsp;</span>
				      <span class="user-infosex" style="color:#000000;margin-left: 100px;">&nbsp;&nbsp;<%=user.getSex() %></span>
	...
	$(function(){
		...
		$(".change ").hover(function(){
			...
			$(this).find(".changeimgtext").stop().animate({left:'0'}, {duration: 100})
		},
		function(){
			$(this).find(".rsp").stop().fadeTo(100,0)
			$(this).find(".changeimgtext").stop().animate({left:'100'}, {duration: "fast"})
			$(this).find(".changeimgtext").animate({left:'-100'}, {duration: 0})
		});
	});
	</script>
	<script >
         function edit(obj){
		   ...
		   if(o.childNodes[0].value) {
		    ...
		    o.innerHTML="<input type='text' style='width=300px;' value='"+o.innerHTML+"' />";
